...
package enum RetryProcessor {
    package static let minBackoffSeconds: TimeInterval = 6 * 3600
    package static let maxAttempts = 7
    package static let permanentAfterDays: TimeInterval = 7 * 86400

    package static func eligibleEntries(_ queue: [RetryQueueEntry], now: Date = Date()) -> [RetryQueueEntry] {
    ...
    package static func shouldMarkPermanent(_ entry: RetryQueueEntry, now: Date = Date()) -> Bool {
