...
package enum MarkdownRenderer {
    package static let chunkSeconds: Double = 150

    package static func render(meta: VideoMetadata, transcript: Transcript) -> String {
    ...
    package static func chunk(_ segments: [Segment], window: Double = chunkSeconds) -> [[Segment]] {
    ...
    package static func formatTimestamp(_ seconds: Double) -> String {
        let s = Int(seconds)
        let h = s / 3600
        let m = (s % 3600) / 60
        let sec = s % 60
    ...
    /// "20240315" → "2024-03-15"
    package static func formatDate(_ uploadDate: String?) -> String? {
        guard let d = uploadDate, d.count == 8 else { return nil }
        let chars = Array(d)
        return "\(String(chars[0..<4]))-\(String(chars[4..<6]))-\(String(chars[6..<8]))"
    }

    package static func formatViewsCompact(_ n: Int?) -> String {
    ...
    package static func formatViewsFull(_ n: Int?) -> String {
        guard let n else { return "—" }
...
extension DateFormatter {
    package static let iso8601Date: DateFormatter = {
        let f = DateFormatter()
